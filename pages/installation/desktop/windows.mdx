---
title: "Windows Installation"
description: "Windows Installation guide for TeamplGPT"
---

import { Callout, Cards } from "nextra/components";
import Image from "next/image";

<Image
  src="/images/getting-started/installation/windows/header-image.png"
  height={1080}
  width={1920}
  quality={100}
  alt="TeamplGPT Windows Installation"
/>

# Windows Installation

There are **two** ways to install TeamplGPT on Windows

**[1. Install using the Installation `.exe` file](#install-using-the-installation-file)**

**[2. Install using Docker](#install-using-docker)**

## Install using the installation file

<Callout type="warning" emoji="⚠️">
  **Application is not signed!**

➤ The TeamplGPT windows application is currently unsigned and Windows defender or other anti-virus software will sometimes flag the application as malicious.

➤ If you do not want to bypass that alert for any reason - please use TeamplGPT another way.

</Callout>

<Callout type="warning" emoji="️⚠️">
  **OPERATING SYSTEM NOTICE**

➤ TeamplGPT is intended to be used on an admin account of Windows Home. Other versions of windows (Enterprise or Server) may not work. We target for Windows 11.

</Callout>

## Downloading the installation file

Here is the download link for the latest version of TeamplGPT Windows.

<Card
  title="Windows 10+ (Home, Professional - 64-bit CPU)"
  href="https://s3.us-west-1.amazonaws.com/public.useanything.com/latest/AnythingLLMDesktop.exe"
></Card>

export const Card = Object.assign(
  // Copy card component and add default props
  Cards.Card.bind(),
  {
    displayName: "Card",
    defaultProps: {
      image: false,
      arrow: true,
      target: "_blank",
    },
  }
);

Your internet browser may need you to verify you want to download and run the TeamplGPT Desktop app since it may be marked as "untrusted" depending on your browser security settings.

Click "**Keep**" when downloading to allow the file to download.

<Image
  src="/images/getting-started/installation/windows/browser-warning.png"
  height={1080}
  width={1920}
  quality={100}
  alt="TeamplGPT Windows Install Browser Warning"
/>

## Installing the application

After downloading the windows `exe` installer for TeamplGPT, you can double-click the installer and it will display the installation process.

<Callout type="info" emoji="️💡">
  **Anti-Virus false positive**

➤ Since the application is unsigned. Native windows defender will want to ensure you mean to execute this application. Click on "more details" and the **"Run anyway"** button will be visible.

**➤ This alert is temporary until the application signing process is completed on our end.**

</Callout>

<Image
  src="/images/getting-started/installation/windows/anti-virus-warning.png"
  height={1080}
  width={1920}
  quality={100}
  alt="TeamplGPT Windows Install Anti Virus Warning"
/>

After which, now the regular installer can run to install TeamplGPT Desktop!

<Image
  src="/images/getting-started/installation/windows/install.png"
  height={1080}
  width={1920}
  quality={100}
  alt="TeamplGPT Windows Install"
/>

Click on the application name "**TeamplGPT**" on your desktop to boot up TeamplGPT!

<Image
  src="/images/getting-started/installation/windows/desktop.png"
  height={1080}
  width={1920}
  quality={100}
  alt="TeamplGPT Windows Open"
/>

---

## Install using Docker

Make sure you have installed [Docker](https://www.docker.com/) on your machine, if you don't have Docker installed then you can install it by following this [guide](https://docs.docker.com/desktop/install/windows-install/)

Run this command on your terminal (powershell or command prompt)

```shell copy
 docker pull mintplexlabs/anythingllm
```

It will pull in the latest TeamplGPT image from docker.

Once you have pulled the latest image, run the following commands on your powershell

```powershell copy showLineNumbers
 $env:STORAGE_LOCATION="$HOME\Documents\anythingllm"; `
 If(!(Test-Path $env:STORAGE_LOCATION)) {New-Item $env:STORAGE_LOCATION -ItemType Directory}; `
 If(!(Test-Path "$env:STORAGE_LOCATION\.env")) {New-Item "$env:STORAGE_LOCATION\.env" -ItemType File}; `
 docker run -d -p 3001:3001 `
 --cap-add SYS_ADMIN `
 -v "$env:STORAGE_LOCATION`:/app/server/storage" `
 -v "$env:STORAGE_LOCATION\.env:/app/server/.env" `
 -e STORAGE_DIR="/app/server/storage" `
 mintplexlabs/anythingllm;
```

The above command will mount the storage locally and run TeamplGPT in Docker

Now go to `http://localhost:3001` on your browser and you are now using TeamplGPT!

All your data and progress will persist between container rebuilds or pulls from Docker Hub.
